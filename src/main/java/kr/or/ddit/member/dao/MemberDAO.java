 ...
 *
 */
@Mapper //매퍼를 갖고있는지 아닌지 분류하기 위한 코드	 
public interface MemberDAO {
	 ...
	 */
	public int insertMember(MemberVO member); //return 타입이 int인 이유는 카운트하려고
	/**
	 * 회원 목록 조회
	 * @param pagingVO TODO
	 * @return size == 0 인 경우, 조건에 맞는 레코드 없음.
	 */
	public List<MemberVO> selectMemberList(PagingVO<MemberVO> pagingVO); //회원 목록 이기에 list
	/** 
	 * 회원 상세 조회
	 * @param memId
	 * @return 조건에 맞는 레코드 없는 경우, null 반환
	 */
	public MemberVO selectMember(@Param("memId") String memId); //vo를 통해 멤버 검색
	/**
	 * 회원 정보 수정
	 * @param member
